...
func chapter1_ep2_script():
	...
	await novel_interface.show_dialog("Lucky！~总算在闭馆之前赶到了！",GameConfig.player_name)
	await novel_interface.show_dialog("大厅的电梯居然全都停运了，好在发现了还在运行的备用货梯。",GameConfig.player_name)
	novel_interface.change_expression("happy")
	await novel_interface.show_dialog("总之，只要去听个讲座盖章就能拿到学分了吧？",GameConfig.player_name)
	novel_interface.change_expression("worry")
	await novel_interface.show_dialog("不过……这层怎么也一个人都看不到？",GameConfig.player_name)
	...
	await novel_interface.show_dialog("讲座不会已经结束了吧？",GameConfig.player_name)
	await novel_interface.hide_character()
	await novel_interface.show_text_only("我一边找讲座的『会议厅』，一边拐到一个旋转楼梯边上。")
	novel_interface.show_character("ren_male","worry")
	await novel_interface.show_dialog("惨了惨了，果然不应该玩那个游戏……",GameConfig.player_name)
	...
	await novel_interface.show_dialog("（诶？？好、好大！！！！脸埋进去了！！)",GameConfig.player_name)
	await novel_interface.show_dialog("（什么状况！！这个人是谁！？）",GameConfig.player_name)
	...
	await novel_interface.show_dialog("（科学家……她是研所工作人员吗？）",GameConfig.player_name)
	...
	await novel_interface.show_dialog("（还有刚才跑过去的那个人似乎蒙着脸，手里还拿着……枪？）",GameConfig.player_name)
	await novel_interface.show_dialog("这里被入侵了哟，不想死就别动。", "穿白大褂的女性")
	await novel_interface.show_dialog("（入、入侵？什么状况……)",GameConfig.player_name)
	...
	await novel_interface.show_dialog("（仔细一看，这个姐姐很漂亮啊！）\n（而且身材也好像模特一样……）",GameConfig.player_name)
	novel_interface.change_expression("normal")
	await novel_interface.show_dialog("（但是，她胸前的名牌上分明写着“科学家”）",GameConfig.player_name)
	novel_interface.change_expression("speechless")
	await novel_interface.show_dialog("（所以刚才的事一定是幻觉吧！）",GameConfig.player_name)
	await novel_interface.hide_character()
	...
	await novel_interface.show_dialog("（我不要自己骗自己了啊！！！）",GameConfig.player_name)
	novel_interface.change_expression("normal")
	...
	await novel_interface.show_dialog("莉琉……莉琉·冯·布劳恩？！",GameConfig.player_name)
	await novel_interface.show_dialog("你、你不就是我要盖章的讲座的主讲人吗！\n还是元老院评定的SS级科学家！",GameConfig.player_name)
	...
	await novel_interface.show_dialog("唔……实在抱歉！！",GameConfig.player_name)
	...
	await novel_interface.show_dialog("可、可是……你一个人……",GameConfig.player_name)
	await novel_interface.hide_character()
	novel_interface.show_character("liliu_research","angry")
	await novel_interface.show_dialog("听明白了吗！", "莉琉")
	await novel_interface.hide_character()
	novel_interface.show_character("ren_male","shock")
	await novel_interface.show_dialog("好的！！",GameConfig.player_name)
