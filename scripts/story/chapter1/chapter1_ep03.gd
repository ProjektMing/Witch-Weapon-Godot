...
func chapter1_ep3_script():
	...
	await novel_interface.show_dialog("糟了，在这里躲不过的……",GameConfig.player_name)
	await novel_interface.show_dialog("他们的视线没有死角可以溜走，唯一的机会是趁他们分开偷袭其中一个",GameConfig.player_name)
	await novel_interface.show_dialog("大概没问题吧……\n既然那个女人赤手空拳都能撂倒对方……",GameConfig.player_name)
	novel_interface.change_expression("normal")
	await novel_interface.show_dialog("得找个东西作为武器……",GameConfig.player_name)
	await novel_interface.hide_character()
	await novel_interface.show_text_only("走廊的墙壁上挂着一根金属球棒，底下铭牌的小字上写着“莱博娜 自由光……”")
	novel_interface.show_character("ren_male","happy")
	await novel_interface.show_dialog("应该是展品吧……\n只是临时借用，嗯嗯，临时借用……",GameConfig.player_name)
	...
	await novel_interface.show_dialog("这和游戏里不一样啊！！",GameConfig.player_name)
	await novel_interface.hide_character()
	novel_interface.show_character("soldier_2")
